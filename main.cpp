...
int menumenu() {
	...
	menu = SDL_LoadBMP("images/menu_image.bmp");
	if (menu == NULL) {
...
void random_obstacle(int x, int rand, SDL_Rect p, bullet B[]) {
	if (rand == 2) {
		spawn[x] = 1;
		destO[x].x = x;
	}
...
void highscore(int t) {
	FILE* fp;
	if (fopen_s(&fp, "highscore.txt", "r") != 0) {
	...
	FILE* fw;
	if (fopen_s(&fw, "highscore.txt", "w") != 0) {
		// Handle the error when opening the file
...
int main(int argc, char* args[])
{
	...
	/*Creating a Window*/
	window = SDL_CreateWindow("Game Project",
	...
	player = SDL_LoadBMP("images/Virtual pet.bmp");
	if (player == NULL) {
		printf("Error Creating Player\n");
		SDL_DestroyRenderer(renderer);
		SDL_DestroyWindow(window);
...
here:
	...
	if (menu_menu == 0) {
		...
		int initial = 0;
		int time = 0;
		// Initialize each bullet in the array
	...
	if (menu_menu == 2) {
		/*Loading the Instruction image into memory using SDL Image Library Function*/
		instruction = SDL_LoadBMP("images/game_instraction.bmp");
		/*Loading the Instruction Image Data Into the Graphics Hardware's Memory*/
	...
	if (menu_menu == 3) {
		...
		credit = SDL_LoadBMP("images/credits.bmp");
